...
with Timer() as timer:
    ...
    for i, fdp_nominal in enumerate(fdp_nominals):
        ...
        test_scores = (threshold_1+threshold_2)/2 - Ypredreg_test
...
out_dir = os.path.join('result-signed2', f'{dataset_name} {args.sample:.2f}')

if not os.path.exists(out_dir):
    os.makedirs(out_dir)

all_res.to_csv(os.path.join('result-signed2', f'{dataset_name} {args.sample:.2f}', f'{dataset_name} {args.sample:.2f} {args.seed}.csv'))
...
with Timer() as timer:
    ...
    for i, fdp_nominal in enumerate(fdp_nominals):
        ...
        test_scores = ((threshold_1+threshold_2)/2 - Ypredreg_test)/rmsereg_test
        # test_scores = (Ytest - Ypredreg_test)/rmsereg_test
        BH_2clip = BH(calib_scores_2clip, test_scores, fdp_nominal)
        fdp, power, cost, powers = eval_n(Ytest, BH_2clip, threshold_1, threshold_2)
