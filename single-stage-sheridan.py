...
with Timer() as timer:
    ...
    for fdp_nominal in fdp_nominals:  # Iterate through each value in ofdr_nominals
        ...
        if min_R_1 is not None and min_R_2 is not None:
            ...
            # Print the results
            # print(sheridan_15)
            print(f"Corresponding R_min_1 value: {min_R_1:.4f}, Corresponding R_min_2 value: {min_R_2:.4f},FDP nominal value: {fdp_nominal:.4f}, Test FDP2 value: {fdp_1:.4f}, Test FDP2 value: {fdp_2:.4f}, Test first-stage power value: {power1:.4f},Test second-stage power value: {power2:.4f}")

        # Reset min_R for the next iteration
        min_R_1 = None  # Initialize as None
        min_R_2 = None
        # try:
        #     fdps_1new2.append(fdp_1)
        # except NameError:
        #     print("Error: 'fdp_1' is not defined. Check your logic before appending.")
        #     raise
        fdps_1new2.append(fdp_1)
...
with Timer() as timer:
    ...
    z_calib1_1 = (np.maximum(threshold_1 - Ypred_calib1, Ypred_calib1 - threshold_2)) / rmse_calib1
    # z_calib1_2 = (Ypred_calib1 - threshold_2) / rmse_calib1
    ...
    var_test = np.var(all_Ypred, axis=1)
    rmse_test = rf_rmse.predict(np.column_stack((Ypred_test, var_test)))
    z_test_1 = (np.maximum(threshold_1 - Ypred_test,Ypred_test - threshold_2 )) / rmse_test
    # z_test_2 = (Ypred_test - threshold_2) / rmse_test
    min_R_1 = None  # Initialize as None
    # min_R_2 = None
    for i, fdp_nominal in enumerate(fdp_nominals):
        ...
        if min_R_1 is not None:
            ...
            # Print the results
            # print(sheridan_15)
            # print(f"Corresponding R_min_1 value: {min_R_1:.4f}, Corresponding R_min_2 value: {min_R_2:.4f},FDP nominal value: {fdp_nominal:.4f}, Test FDP value: {fdp:.4f}, oFDP nominal value: {fdp_nominal:.4f}, Test oFDP value: {fdp:.4f}, Test first-stage power value: {power1:.4f},Test second-stage power value: {power2:.4f}")
...
out_dir = os.path.join('result-1', f'{dataset_name} {args.sample:.2f}')

if not os.path.exists(out_dir):
    os.makedirs(out_dir)

all_res.to_csv(os.path.join('result-1', f'{dataset_name} {args.sample:.2f}', f'{dataset_name} {args.sample:.2f} {args.seed}.csv'))
