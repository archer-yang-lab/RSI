...
for name in dataset_list:
    ...
    for j in range(1, 1 + n_itr):
        df = pd.read_csv(
            os.path.join("result-trans", f"{name} {sample:.2f}", f"{name} {sample:.2f} {j}.csv"))
        df_ones.append(df)
    df = pd.concat(df_ones).groupby("fdp_nominals", as_index=False).mean()
    df_list.append(df)

out_dir = os.path.join('figure-trans')
...
for i, name in enumerate(dataset_list):
    ...
    if i == 0:
        ...
        line1, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdpreg_cs'],
                         label='regression', marker='o', color='steelblue', alpha=0.8)

        line2, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdpn_cs'],
                         label='binarization + classification', marker='o', color='orange', alpha=0.8)

        line3, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdpbreg_cs'],
                         label='binarization + regression', marker='o', color='darkgreen', alpha=0.8)
        #
        ...
        ax.legend(loc='best', bbox_to_anchor=(5.4, -3.0), frameon=True, shadow=False, ncol=3, fontsize=22)
    else:
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdpreg_cs'],
                         label='regression', marker='o', color='steelblue', alpha=0.8)

        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdpn_cs'],
                         label='binarization + classification', marker='o', color='orange', alpha=0.8)

        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['fdpbreg_cs'],
                         label='binarization + regression', marker='o', color='darkgreen', alpha=0.8)
        # 
...
# Display the plot
# plt.savefig(os.path.join("figure-trans", "compfdp3_{model}.png"))
plt.savefig(os.path.join("figure-trans", "compfdptrans.png"))
# plt.show()
...
for i, name in enumerate(dataset_list):
    ...
    if i == 0:
        # Plot data for each model and conformal method
        line1, = ax.plot(df_list[i]['fdpreg_cs'], df_list[i]['powerreg_cs'],
                         label='regression', marker='o', color='steelblue', alpha=0.8)

        line2, = ax.plot(df_list[i]['fdpn_cs'], df_list[i]['powern_cs'],
                         label='binarization + classification', marker='o', color='orange', alpha=0.8)

        line3, = ax.plot(df_list[i]['fdpbreg_cs'], df_list[i]['powerbreg_cs'],
                         label='binarization + regression', marker='o', color='darkgreen', alpha=0.8)
        # 
        ...
        ax.plot(df_list[i]['fdpreg_cs'], df_list[i]['powerreg_cs'],
                         label='regression', marker='o', color='steelblue', alpha=0.8)
        ax.plot(df_list[i]['fdpn_cs'], df_list[i]['powern_cs'],
                         label='binarization + classification', marker='o', color='orange', alpha=0.8)
        ax.plot(df_list[i]['fdpbreg_cs'], df_list[i]['powerbreg_cs'],
                         label='binarization + regression', marker='o', color='darkgreen', alpha=0.8)
...
# Display the plot
plt.savefig(os.path.join("figure-trans", "comppowerobtrans.png"))
# plt.show()
...
for i, name in enumerate(dataset_list):
    ...
    if i == 0:
        # Plot data for each model and conformal method
        line1, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powerreg_cs'],
                         label='regression', marker='o', color='steelblue', alpha=0.8)

        line2, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powern_cs'],
                         label='binarization + classification', marker='o', color='orange', alpha=0.8)

        line3, = ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powerbreg_cs'],
                         label='binarization + regression', marker='o', color='darkgreen', alpha=0.8)
        # 
        ...
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powerreg_cs'],
                         label='regression', marker='o', color='steelblue', alpha=0.8)
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powern_cs'],
                         label='binarization + classification', marker='o', color='orange', alpha=0.8)
        ax.plot(df_list[i]['fdp_nominals'], df_list[i]['powerbreg_cs'],
                         label='binarization + regression', marker='o', color='darkgreen', alpha=0.8)
...
# Display the plot
plt.savefig(os.path.join("figure-trans", "comppowertrans.png"))
# plt.show()
...
# Display the plot
# plt.savefig(os.path.join("figure-trans", "compfdp3_{model}.png"))
plt.savefig(os.path.join("figure-trans", "compfdp0.1trans.png"))
# plt.show()
...
# Display the plot
plt.savefig(os.path.join("figure-trans", "comppowerob0.1trans.png"))
# plt.show()
...
# Display the plot
plt.savefig(os.path.join("figure-trans", "comppower0.1trans.png"))
# plt.show()